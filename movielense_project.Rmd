...
```{r}
# Following three lines of code can be used to Seperate the "genres" column in the edx & validation dataset, and add each genre into new separate row. The genre in this project is not seprated due to machine memory limit issue.
...
### Limitation
> One serious limitation was the machine memory, which was disabeling the code execution. Serious bottleneck was observed when sperating the various genres from each row to the new rows in th etraining dataset. Therefore the current project was done without seprating the genres into rows.
> Another limit was the internet download speed which was taking longer time to download the data. 
...
[1]: Material provided in the machine learning course of the edx (HarvardX: PH125.8x).
